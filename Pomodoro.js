...
state = {
    TempMinutes:25,
    TempSeconds:0,
    countDown:moment.duration().add({minutes:25,seconds:0}),
    running: false,
    tempCount: false
    };
    Timer = null
    ...
    resetTimer = () =>{
      if (!this.state.running){
        console.log("Reset")
        clearInterval(this.Timer)
        this.setState({TempMinutes:25,TempSeconds:0,countDown:moment.duration().add({minutes:25,seconds:0}),tempCount:false})
      }}
